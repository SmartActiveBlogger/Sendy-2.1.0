...
<?php 
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
	    ...
	    //Check if bounces and complaints wasn't detected
	    if(get_bounced()==0 && get_bounced('soft')==0 && $bounce_setup==0 && get_app_data('bounce_setup')==0) $bounce_detection = false;
	    else $bounce_detection = true;
	    if(get_complaints()==0 && $complaint_setup==0 && get_app_data('complaint_setup')==0) $complaint_detection = false;
	    else $complaint_detection = true;
	}
?>
    ...
    $(document).ready(function() {
        ...
        chart = new Highcharts.Chart({
            ...
            series: [
            {
                ...
                <?php if(!$complaint_detection) echo ',visible: false';?>
            },
            {
                name: '<?php echo _('Bounced');?>',
                data: [<?php echo get_bounced();?>]
                <?php if(!$bounce_detection) echo ',visible: false';?>
            },
            {
    	...
    	<div class="row-fluid">
	    	...
	    	<div class="span6">
	    		<div class="well">
			    	...
			    	<?php else:?>
			    	<h3><span class="badge badge-inverse" style="font-size:16px;"><?php echo _('No bounces were registered');?></span></h3><br/>
			    	<?php endif;?>
			    	
			    	<?php if($complaint_detection):?>
			    	...
			    	<?php else:?>
			    	<h3><span class="badge badge-inverse" style="font-size:16px;"><?php echo _('No complaints were registered');?></span></h3>
			    	<?php endif;?>
			    </div>
	    	</div>
	    </div>
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	...
				  	else
				  	{
			  			...
			  			if($bounce_detection) echo '<td>'._('No emails bounced from this campaign!').'</td>';
			  			echo '<td>'._('No bounces were registered').'</td>';
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	...
				  	else
				  	{
			  			...
			  			if($complaint_detection) echo '<td>'._('No one marked your email as spam!').'</td>';
			  			echo '<td>'._('No complaints were registered').'</td>';
